...
{
    public void Create(string store, string tableName, string primaryKey, string columnName, string value)
    {
        var encryptKey = _bukalemunOptions.Value.Stores[store].EncryptKey;
        var encrypted = _cryptoProvider.Encrypt(encryptKey, value);

        _camouflageRepository.Upsert(new Data.Entites.Camouflaged()
        {
            Store = store,
            TableName = tableName,
